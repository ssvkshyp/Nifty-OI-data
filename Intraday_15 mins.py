          ...
          for k in Ticker:
              ...
              with requests.Session() as req:
                  ...
                  if not  data1:
                      ...
                      spot= int(final_data['underlyingValue'].unique())
                      ...
                      call_put_OI['Spot']=int(spot)
                      columns=['CE', 'PE', 'Diff', 'time']
                      if(Path.exists(Path("E:\code and finance\Data\Intraday","NIFTY", date+".xlsx"))):
                           hist=pd.read_excel(Path("E:\code and finance\Data\Intraday","NIFTY", date+".xlsx"),usecols=columns)                
                           hist=hist.append(call_put_OI)
                           hist.to_excel(Path("E:\code and finance\Data\Intraday","NIFTY", date+".xlsx"))
                           ...
                           plt.plot(hist['time'],hist['Spot'], color='black', label="PE")
                           plt.show()
                           print(hist)
                      else:
                           call_put_OI.to_excel(Path("E:\code and finance\Data\Intraday","NIFTY", date+".xlsx")) 

                      
                      call_put_OI.to_pickle(Path("E:\code and finance\Data\Intraday","NIFTY", date+".pkl") )
                      t.sleep(60*14)          
    Hour = dt.datetime.now().hour
